...
import ru.bukivadis.myfirstapp.utils.DateUtils
...
class PostDaoImpl(private val db: SQLiteDatabase) : PostDao {
    ...
    private fun postToContentValues(post: Post): ContentValues {
        ...
        val date = cal.time
        val sdf = SimpleDateFormat("dd MMMM в HH:mm", Locale.getDefault())
        val formatted = sdf.format(date)
        return ContentValues().apply {
            put(Columns._ID, post.id)
            put(Columns.AUTHOR, "Я")
            put(Columns.AUTHOR_ID, post.authorId)
            put(Columns.CONTENT, post.content)
            put(Columns.PUBLISHED, formatted)
